...
public class conveyor : MonoBehaviour
{
	...
	{
		if(other.gameObject.layer == 7) // only move trash
		{
			//other.gameObject.GetComponent<trash>().move = true;
			// get direction of endpoint from cur pos, then apply given speed over time
			other.transform.position = Vector3.MoveTowards(other.transform.position, new Vector3(endPoint.transform.position.x, endPoint.transform.position.y, endPoint.transform.position.z), managerRef.GetComponent<spawnTrash>().currentSpeed * Time.deltaTime);
